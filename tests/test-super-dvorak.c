...
int main() {
  ...
  test(&state, PRESSED, KC_SCOLON, 1, (struct event[]) { { PRESSED, KC_S } });
  test(&state, RELEASED, KC_SCOLON, 1, (struct event[]) { { RELEASED, KC_S } });
  test(&state, PRESSED, KC_LSHIFT, 1, (struct event[]) { { PRESSED, KC_LSHIFT } });
  test(&state, PRESSED, KC_CAPSLOCK, 0, (struct event[]) { });
  test(&state, PRESSED, KC_SCOLON, 3, (struct event[]) { { RELEASED, KC_LSHIFT }, { PRESSED, KC_RALT }, { PRESSED, KC_S } });
  test(&state, RELEASED, KC_SCOLON, 2, (struct event[]) { { RELEASED, KC_S }, { RELEASED, KC_RALT } });
