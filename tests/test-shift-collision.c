...
static const struct mapping our_mappings[] = {
  // A 0
  { .from_modifiers = 0x0, .from_absorbing_modifiers = 0x0, .to_modifiers = LEFT_SHIFT_MASK, .to_action = KC_C},
  // B 0
  { .from_modifiers = 0x0, .from_absorbing_modifiers = 0x0, .to_modifiers = 0x0, .to_action = KC_D},
};

static const struct key_definition our_key_definitions[] = {
  [KC_A] = { action_key_style, .action_key = { .mappings_start = 0, .mappings_end = 1 } },
  [KC_B] = { action_key_style, .action_key = { .mappings_start = 1, .mappings_end = 2 } },
};
...
int main() {
  ...
  test(&state, PRESSED, KC_A, 2, (struct event[]) { { PRESSED, KC_LSHIFT }, { PRESSED, KC_C } });
  test(&state, PRESSED, KC_B, 2, (struct event[]) { { RELEASED, KC_LSHIFT }, { PRESSED, KC_D } });
}
