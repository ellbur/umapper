...
static const struct mapping our_mappings[] = {
  // SPACE 0
  { .from_modifiers = 0x0, .from_absorbing_modifiers = 0x0, .to_modifiers = 0x0, .to_action = KC_SPACE },
};

static const struct key_definition our_key_definitions[] = {
  [KC_SPACE] = { action_key_style, .action_key = { .mappings_start = 0, .mappings_end = 1 } },
...
int main() {
  ...
  test(&state, PRESSED, KC_SPACE, 1, (struct event[]) { { PRESSED, KC_SPACE } });
}
